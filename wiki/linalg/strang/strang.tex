...
\vspace{1mm}\\
We know that the projection $\bm p$ will be some multiple of $\bm a$; call it $\bm p=\hat{\bm x}\bm a$ (so here $\hat{\bm x}$ denotes a constant)---computing this number 
$\hat{\bm x}$ will give us the vector $\bm p$. We can call the perpendicular line $\bm b-\bm p$ the `error' $\bm e=b-\hat{\bm x}\bm a$, and use the fact that it is 
perpendicular to $\bm a$ to derive $\hat{\bm x}$:
\begin{center}
\includegraphics[width=6cm]{54}\\
\includegraphics[width=11cm]{55}
\end{center}
With that we can compute the projection $\bm p=\hat{\bm x}\bm a$:
\begin{center}
...
\begin{equation*}
\bm p=\hat{\bm x}\bm a=\frac{\bm a^T\bm b}{\bm a^T\bm a}\bm a
\end{equation*}
...
$\bm{Pb}=\bm p$. This can found easily; since $\hat{\bm x}$ is a constant, we can rearrange the above equation:
\begin{center}
