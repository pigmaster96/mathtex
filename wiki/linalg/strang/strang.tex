...
\section{Every rank $r$ matrix is a sum of $r$ rank one matrices}
Rank one matrices have the form $\bm{uv^T}$. Recall the $\bm A=\bm{CR}$ factorisation,
where we reduce the matrix by row operations to $\bm R_0$, which has the same row space as $\bm A$, and remove the zero rows to find $\bm R$, which also has the same row
space. For instance:
\begin{center}
\includegraphics[width=10cm]{46}
\end{center}
For each independent column in $\bm A$, a new independent column is added to $\bm C$ and a new indentity component added to $\bm R$. On the other
hand for each dependent column in $\bm A$, only a new column is added to $\bm R$ and $\bm C$ is left unchanged. Each independent column in $\bm C$ 
contributes a rank 1 matrix:
