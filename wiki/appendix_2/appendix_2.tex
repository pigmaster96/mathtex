...
\subsection{Autonomous equations, Logistic Model}
...
\dot{y}=3y
\end{equation*}
First we find the critical points; see that only one critical point exists: $y=0$. 
Next we plot the graph of $f(y)$ (in this case a straight line); see that $\dot{y}>0$ for $y>0$ and $\dot{y}<0$ for $y<0$:
\begin{figure}[h]
\begin{center}
\includegraphics[width=10cm]{17}\\
\end{center}
\end{figure}\\
With this we can draw the phase line. 
