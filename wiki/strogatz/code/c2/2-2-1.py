...
def rk4logistic(init_cons,dt):
    ...
    tvec=np.arange(0,1,dt,dtype=np.float128)
    #vector for values
...
#compute
t,x=rk4logistic([(0,0.5),(0,-4),(0,1.99),(0,2.01),(0,2),(0,-2)],0.01)
...
hlim=(0,1)
vlim=(-4,4)
X=np.arange(-4,4,0.4,dtype=np.float128)
T=np.arange(0,1,0.05,dtype=np.float128)
...
ax.quiver(T,X,traj_t,traj_x,pivot='tail',scale=600,headwidth=1)
